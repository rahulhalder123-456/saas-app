...
// ─── Permission Check for Creating Companions ─────────────────────────────────
...
export const addBookmark = async (companionId: string, _path?: string) => {
  const { userId } = await auth();
  if (!userId) return;
...
export const removeBookmark = async (companionId: string, _path?: string) => {
  const { userId } = await auth();
  if (!userId) return;
